...
function onInteriorRelocateMove(x, y, z, toPosition, undo)
    ...
    if rootElement and isElement(rootElement) then
        ...
        local midpointX, midpointY, midpointZ = Maths.calculateInteriorMidpoint(objects);
        local aX, aY, aZ = Maths.calculateMovingObjectAxisAdditions(Vector3(x, y, z), midpointX, midpointY, midpointZ);
        if not undo then
            if toPosition then
                pushMoveAction(aX, aY, aZ);
            else
                pushMoveAction(x, y, z);
            end
        end
